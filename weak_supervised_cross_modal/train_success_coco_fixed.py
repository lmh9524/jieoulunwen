"""
修复版本的基于成功训练经验的COCOAttributes训练脚本
"""
import sys
import os
import torch
...
class COCOAttributesFixedDataset(Dataset):
    """修复版本的COCO Attributes数据集"""
...
def main():
    """主训练函数"""
    parser = argparse.ArgumentParser(description='修复版本的COCOAttributes训练')
    parser.add_argument('--epochs', type=int, default=40, help='训练轮数')
    parser.add_argument('--lr', type=float, default=1e-4, help='学习率')
    parser.add_argument('--batch_size', type=int, default=16, help='批次大小')
    parser.add_argument('--save_dir', type=str, default='./checkpoints_fixed', help='保存目录')
    parser.add_argument('--log_dir', type=str, default='./logs_fixed', help='日志目录')
    
    args = parser.parse_args()
    
    # 设置日志
    log_file = setup_logging(args.log_dir)
    logging.info("开始修复版本的COCOAttributes训练...")
    ...
    try:
        ...
        # 创建数据集
        train_dataset = COCOAttributesFixedDataset(
        ...
        val_dataset = COCOAttributesFixedDataset(
            'D:/KKK/jieoulunwen/data/cocottributes-master/cocottributes-master/MSCOCO/cocottributes_new_version.jbl',
        ...
        for epoch in range(args.epochs):
            ...
            logging.info(f"Epoch {epoch+1}: Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Time: {epoch_time:.2f}s")
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'config': config
                }, os.path.join(args.save_dir, 'best_coco_attributes_fixed.pth'))
